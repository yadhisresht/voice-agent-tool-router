...
def clean_entity(entity_text):
    if not entity_text:
        return None
    cleaned = re.sub(r'^\s*(the|a|an|and|or|to|for|at|in|from)\s+', '', entity_text.strip(), flags=re.IGNORECASE)
    cleaned = re.sub(r'[?.,!]\s*$', '', cleaned).strip()
    return cleaned if cleaned else None

def ner_based_plan_tasks(query):
    segments = re.split(r'\s+(?:and|then|after that|;|,)\s+', query.lower().strip())
    plan = []
    ...
    NAV_START_PATTERN = r'\bfrom\s+([a-zA-Z0-9\s]+?)(?=\s*(?:to|and|what|where|$))'
    NAV_DEST_PATTERN_1 = r'\bto\s+([a-zA-Z0-9\s]+?)(?=\s*(?:from|and|what|where|$))'
    NAV_DEST_PATTERN_2 = r'\b(?:navigate|route|go to|take me to)\s+([a-zA-Z0-9\s]+?)(?=\s*(?:from|and|what|where|$))'
    
    WEATHER_LOC_PATTERN = r'\b(?:weather|forecast)(?:\s+(?:in|at|for))?\s+([a-zA-Z0-9\s]+?)(?=\s*(?:and|what|where|$))'
    
    CALL_CONTACT_PATTERN = r'\b(?:call|phone|ring)\s+([a-zA-Z\s]+?)(?=\s*(?:and|what|where|$))'
    
    PLAY_ITEM_PATTERN = r'\bplay\s+([a-zA-Z0-9\s]+?)(?=\s*(?:and|what|where|$))'
    
    for i, segment in enumerate(segments):
        task = {"tool_name": None, "invocation_order": i + 1, "parameters": {}}
        
        if re.search(r'(navigate|route|go to|take me to|distance)', segment):
            task["tool_name"] = tool_map["navigate"]
            
            match_start = re.search(NAV_START_PATTERN, segment)
            start_location = clean_entity(match_start.group(1)) if match_start else None

            destination = None
            match_dest_1 = re.search(NAV_DEST_PATTERN_1, segment)
            match_dest_2 = re.search(NAV_DEST_PATTERN_2, segment)
            
            if match_dest_1:
                destination = clean_entity(match_dest_1.group(1))
            elif match_dest_2:
                destination = clean_entity(match_dest_2.group(1))
                
            task["parameters"]["start_location"] = start_location if start_location else "current vehicle location"
            task["parameters"]["destination"] = destination if destination else "Unknown destination (NER failure)"
            
            if destination:
                last_destination = destination 
            
        elif re.search(r'(call|phone|ring)', segment):
            task["tool_name"] = tool_map["call"]
            
            match = re.search(CALL_CONTACT_PATTERN, segment)
            contact_name = clean_entity(match.group(1)) if match else None
            
            task["parameters"]["contact_name"] = contact_name if contact_name else "Unknown contact (NER failure)"
    
        elif re.search(r'(weather|forecast)', segment):
            task["tool_name"] = tool_map["weather"]
            
            match = re.search(WEATHER_LOC_PATTERN, segment)
            location = clean_entity(match.group(1)) if match else None
            
            if not location or re.search(r'\b(?:when i reach there|there|that location|the destination)\b', segment):
                if last_destination:
                    location = last_destination 
                else:
                    location = "current vehicle location" 

            task["parameters"]["location"] = location
            
        elif re.search(r'(play|pause|skip|music|song|artist)', segment):
            task["tool_name"] = tool_map["music"]
            
            if re.search(r'pause', segment):
                task["parameters"]["action"] = "pause"
            elif re.search(r'skip', segment):
                task["parameters"]["action"] = "skip"
            else:
                task["parameters"]["action"] = "play" 
            
            if task["parameters"]["action"] == "play":
                match = re.search(PLAY_ITEM_PATTERN, segment)
                item_name = clean_entity(match.group(1)) if match else "last played station/song"
                task["parameters"]["item_name"] = item_name

        if task["tool_name"]:
            plan.append(task)
            
    if not plan:
        return [{"error": "NER-Simulated parser found no matching intent or extractable entities for the query."}]
    
    return plan

def resolve_tools_for_query(query, embeddings, ltr_model_predictor):    
    print("\n--- STAGE 1: Tool Selection & Ranking (LTR/Mock) ---")
    ...
    print("\n--- STAGE 2: NER-Simulated Agent Planning (Entity-Based Execution) ---")
    
    execution_plan = ner_based_plan_tasks(query)

    if isinstance(execution_plan, list) and not execution_plan[0].get("error"):
        
        print("\nSUCCESS: GENERATED EXECUTION PLAN (NER-Simulated)")
        print(f"Plan contains {len(execution_plan)} sequential tasks:")
...
if API_KEY != "AIzaSyASRU-gsPRUnc5yPSYzk6P4DoA_4KJH-1U":
    ...
    print("WARNING: Stage 2 uses simulated NER rules and may fail on complex phrasing.")
